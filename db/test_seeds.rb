...
admin = User.find_by_first("ADMIN") || User.create(:first=>:ADMIN, :last => :ACCOUNT, :email => "admin@gmail.com", :username => :admin, :password => :admin, :status => "active")
...
jason = User.find_by_first("Jia") || admin.create_user(:first=>:Jia, :last=> :Teoh, :email => "jason@gmail.com",  :username=>:jason, :password => :jteoh)

proj1 = Project.find_by_name("MyFirstProject") || admin.create_project(:name=>:MyFirstProject, :description => "This is the first test project")
#proj1.owner = admin
...
Risk.create_risk(admin.id, proj1.id, {:cost=> 1, :schedule => 2, :technical => 3, :status=> "active", :early_impact=> "2013-11-20", :last_impact=> "2013-11-2", :owner_id=>"jason", :description => "Project 1 risk 2", :probability => 3, :title => "Risk 2"})
